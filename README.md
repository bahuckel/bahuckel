...
- **The server never runs plugin code.** It stores the package, validates it, and hands it out to members; it will not execute a DLL, spawn a process or evaluate a script from one. The *client* loads it, in a context of its own that can be unloaded again, and a plugin that throws gets an error panel rather than taking the app down with it.
- **Nothing runs without asking you first.** A plugin is a program: it runs inside the app, as you, with the app's access to your computer. .NET cannot fence code in, and this does not pretend to. So before a plugin runs on your machine you are shown what it is, who added it and its package fingerprint, and asked; the answer is pinned to that exact package, and an update asks again. A plugin that reaches for the obvious escapes — starting processes, native code, raw sockets, loading code at run time — is refused before you are even asked. Only allow plugins from people you trust.
...
### Bahuckel Diag — predictive hardware diagnostics
